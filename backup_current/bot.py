    ...
    try:
        for i, q in enumerate(questions):
            ...
            try:
                ...
                logger.error(f"Error posting quiz {i+1}: {e}")
                await query.edit_message_text(f"⚠️ *Warning:* Failed to post quiz {i+1}. Skipping...", parse_mode="Markdown")
        ...
        await query.edit_message_text(
            f"✅ *Posting Complete!*\n\n"
            f"🏁 Successfully processed `{total}` quizzes for `{channel_id}`.",
            parse_mode="Markdown"
        )
    except Exception as e:
        logger.error(f"Fatal error in post_to_channel_callback: {e}")
        await query.edit_message_text(f"❌ *Fatal Error during posting:* {e}", parse_mode="Markdown")
    finally:
